...
class Abinit(FileIOCalculator):
    ...
    def _is_converged(self):
        filename = self._output_path(self.label + '.txt')
        ...
        try:
            ...
            return False
        if 'was not enough scf cycles to converge' in text:
            return False
        has_energy = ('>>>>>>>>> etotal=' in text
                      or '>>>> etotal (dc)=' in text
                      or '>>>>> internal e=' in text)
        if not has_energy:
            for line in text.split('\n'):
                if 'etotal' in line and ':' in line:
                    has_energy = True
                    break
        if not has_energy:
            try:
                logfile = self._output_path(self.label + '.log')
    ...
    def _has_density(self):
        den_path = self._output_path(self.prefix + 'o_DEN')
        return os.path.isfile(den_path)

    def _setup_restart(self):
        if 'irdwfk' in self.parameters or 'irdden' in self.parameters:
            return
        wfk_output = self._output_path(self.prefix + 'o_WFK')
        wfk_input = self._output_path(self.prefix + 'i_WFK')
        den_output = self._output_path(self.prefix + 'o_DEN')
        den_input = self._output_path(self.prefix + 'i_DEN')
        if os.path.isfile(wfk_output):
            if not os.path.isfile(wfk_input):
                os.symlink(os.path.basename(wfk_output), wfk_input)
            self.parameters['irdwfk'] = 1
        elif os.path.isfile(den_output):
            if not os.path.isfile(den_input):
                os.symlink(os.path.basename(den_output), den_input)
            self.parameters['irdden'] = 1
    ...
    def write_input(self, atoms, properties=None, system_changes=None):
        ...
        # Use absolute paths to avoid needing psp in each subdirectory
        for ppp in self.ppp_list:
            # Write absolute path directly to .files
            fh.write('%s\n' % os.path.abspath(ppp))
        fh.close()
        ...
        inp = {}
        inp.update(param)
        for key in ['xc', 'smearing', 'kpts', 'pps', 'raw', 'gamma', 'reuse']:
            del inp[key]
    ...
    def calculate(self,
        ...
        reuse = self.parameters.get('reuse', True)

        if reuse and self._is_converged():
            print(f"Reusing converged result from {self._output_path(self.label + '.txt')}")
            self.read_results()
            return

        if reuse and self._has_density():
            self._setup_restart()
            print(f"Restarting from existing density in {self.directory}")
        ...
        olddir = os.getcwd()
        if self.dryrun:
            print(f"[DRYRUN] Would run: {command}")
            return
        try:
    ...
    def read(self, label):
        ...
        self.atoms = read_abinit_in(self.label + '.in')
    ...
    def read_results(self):
        ...
        # Fallback: try alternative format if not found (for newer ABINIT versions)
        if efree is None:
            for line in iter(text.split('\n')):
                # Match "etotal    :  -2.53042665e+02"
                if 'etotal' in line and ':' in line:
                    parts = line.split(':')
                    if len(parts) >= 2:
                        try:
                            efree = float(parts[1].strip().split()[0]) * Hartree
                            break
                        except (ValueError, IndexError):
                            pass
        
        # Another fallback: read from .log file if .txt doesn't have energy
        if efree is None:
            try:
                logfile = self.label + '.log'
                with open(logfile) as f:
                    log_lines = f.readlines()
                # Find last ETOT line: " ETOT  9  -253.07598795816    -1.883E-02..."
                for line in reversed(log_lines):
                    if ' ETOT ' in line and len(line.split()) >= 3:
                        parts = line.split()
                        try:
                            # The energy is typically the 3rd field after ETOT
                            etot_idx = parts.index('ETOT')
                            if len(parts) > etot_idx + 2:
                                efree = float(parts[etot_idx + 2]) * Hartree
                                break
                        except (ValueError, IndexError):
                            pass
            except (IOError, FileNotFoundError):
    ...
    def initialize(self, atoms):
        ...
        self.ppp_list = []
        if self.parameters.xc == 'PW':
            xcname = 'PW'
        elif self.parameters.xc != 'LDA' and self.parameters.xc != 'PBEsol':
            xcname = 'PBE'
        ...
        if pps not in [
                ...
                'eric-gen', 'ONCV', 'jth_sp', 'GPAW', 'oncv'
        ]:
        ...
        for Z in self.species:
                ...
                if pps == 'jth':
                    matches = glob(
                        os.path.join(pppaths[-1], '**/%s.%s*JTH.xml' % (
                            symbol, xcdict[xcname])), recursive=True)
                    if not matches:
                        raise RuntimeError(f'No JTH pseudopotential for {symbol}!')
                    name = matches[0]
            ...
            elif pps in ['ONCV', 'oncv']:
                xcdict = {'LDA': 'PW', 'PBE': 'PBE', 'PBEsol': 'PBEsol', 'PW': 'PW'}
                #print '*/%s_%s_%s*'%(number,symbol,xcdict[xcname])
                p=os.path.join(pppaths[-1], 'ONCVPSP-%s-PDv0.4/%s.psp8' %
                                 (xcdict[xcname], symbol))
                print(p)
                name = glob(
                    p)[-1]
            elif pps in ['GPAW']:
    ...
    def read_number_of_electrons(self):
        ...
        for line in open(self.label + '.log'):  # find last one
            if line.find('with nelect') != -1:
                # Extract value between '=' and ',': "with nelect=     70.000000, Number..."
    ...
    def read_nkpt(self):
        ...
        with open(fname) as myfile:
            for line in myfile:
                result = re.findall(r'nkpt\s*=\s*(\d*)', line)
                if len(result) != 0:
                    nkpt = int(result[0])
                    return nkpt
        #if nkpt is None:
    ...
    def read_nband(self):
        ...
        with open(fname) as myfile:
            for line in myfile:
                result = re.findall(r'mband\s*=\s*(\d*)', line)
                if len(result) != 0:
                    nband = int(result[0])
                    return nband
        return nband
